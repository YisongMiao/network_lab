...
void handle_packet(iface_info_t *iface, char *packet, int len)
{
	...
	//lookup
	if(!lookup_port(eh->ether_dhost) && strcmp(packet, "33")){
		broadcast_packet(iface, packet, len);
		//printf("Breadcasted the packet come from: %d\n", iface->fd);
	}
	else if(strcmp(packet, "33")){
		iface_send_packet(lookup_port(eh->ether_dhost), packet, len);
		//printf("Sent the packet from %d to %d\n", iface->fd, lookup_port(eh->ether_dhost)->fd);
	}

	//insert
	if(!lookup_port(eh->ether_shost) && strcmp(packet, "33")){
