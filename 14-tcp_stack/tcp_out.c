...
// send data from buf by calling tcp_send_data
//
// if the amount of data transmitted is larger than snd_wnd, this function
// should sent data partially and wait until snd_wnd allows to send more data
...
#define min3(x,y,z) min(min(x,y),z)
		int seg_size = min3(max_seg_size, len - wlen, tsk->snd_wnd);

		int pkt_size = hdr_size + seg_size;

		packet = malloc(pkt_size);
		if (!packet) {
			log(ERROR, "malloc packet failed when sending data.");
			return -1;
		}
